        ...
        Write-Host "E necessaria conexao com a internet." -ForegroundColor Red
        Pause
        exit
        }
        ...
        Write-Host "madruga" -ForegroundColor White -NoNewline; Write-Host "EXEC`n" -ForegroundColor Cyan

        Write-Host "INSTALANDO 7Z`n" -ForegroundColor Green
...
Remove-Item "$env:ProgramData\Microsoft\Windows\Start Menu\Programs\7-Zip" -Recurse -Force -ErrorAction SilentlyContinue | Out-Null
        ...
        Write-Host "INSTALANDO C++`n" -ForegroundColor Green
...
Start-Process -Wait "$env:SystemRoot\Temp\vcredist2015_2017_2019_2022_x64.exe" -ArgumentList "/quiet /norestart" -WindowStyle Hidden 
        ...
        Write-Host "INSTALANDO DDU`n" -ForegroundColor Green
...
Write-Host "INSTALANDO BRAVE`n" -ForegroundColor Green
## explorer "https://brave.com"
...
Write-Host "INSTALANDO DIRECT X`n" -ForegroundColor Green
...
$madrugaexecPs1 = @'
	    ...
	    # REMOVE WINLOGON madrugaexec PS1 FILE
        ...
        Write-Host "DEFENDER SETTINGS`n" -ForegroundColor Blue
        ## windowsdefender:
...
cmd /c "bcdedit /deletevalue {current} safeboot >nul 2>&1"
        ...
        Write-Host "DDU & REINICIANDO`n" -ForegroundColor Red
...
'@
Set-Content -Path "$env:SystemRoot\Temp\madrugaexec.ps1" -Value $madrugaexecPs1 -Force

# install winlogon madrugaexec ps1 file to run in safe boot
cmd /c "reg add `"HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Winlogon`" /v `"Userinit`" /t REG_SZ /d `"powershell.exe -nop -ep bypass -WindowStyle Maximized -f $env:SystemRoot\Temp\madrugaexec.ps1`" /f >nul 2>&1"

# create madrugaexec2 ps1 file
$madrugaexec2Ps1 = @'
        # SCRIPT RUN AS ADMIN
        If (!([Security.Principal.WindowsPrincipal][Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltInRole]"Administrator"))
        {Start-Process PowerShell.exe -ArgumentList ("-NoProfile -ExecutionPolicy Bypass -File `"{0}`"" -f $PSCommandPath) -Verb RunAs
        ...
        Write-Host "MICROSOFT STORE SETTINGS`n" -ForegroundColor Blue
...
reg unload "HKLM\Settings" >$null 2>&1
		...
		Write-Host "WINDOWS SETTINGS`n" -ForegroundColor Blue
		## regedit
		## control
        ## ms-settings:
        ## ms-settings:privacy
...
Start-Sleep -Seconds 10
        ...
        Write-Host "REMOVENDO EDGE`n" -ForegroundColor Red
...
dism /online /Remove-Package /PackageName:$EdgeLegacyPackage /quiet /norestart 2>$null | Out-Null
        ...
        Write-Host "REMOVENDO UWP APPS`n" -ForegroundColor Red
...
$_.Name -notlike '*windows.immersivecontrolpanel*'
        ...
        Write-Host "REMOVENDO UWP FEATURES`n" -ForegroundColor Red
...
Remove-WindowsCapability -Online -Name $_.Name | Out-Null
        ...
        Write-Host "REMOVENDO LEGACY FEATURES`n" -ForegroundColor Red
...
Disable-WindowsOptionalFeature -Online -FeatureName $_.FeatureName -NoRestart -WarningAction SilentlyContinue | Out-Null
		...
		Write-Host "REMOVENDO LEGACY APPS`n" -ForegroundColor Red
		## appwiz.cpl
...
New-Item -Path "$env:ProgramData\Microsoft\Windows\Start Menu\Programs\StartUp" -ItemType Directory -ErrorAction SilentlyContinue | Out-Null
        ...
        function Show-Menu {
        Clear-Host
        Write-Host "INSTALAR DRIVERS GRAFICOS" -ForegroundColor Yellow
        Write-Host "SELECIONE A GPU DO SEU SISTEMA`n" -ForegroundColor Yellow
        Write-Host " 1.  NVIDIA" -ForegroundColor Green
        Write-Host " 2.  AMD" -ForegroundColor Red
        Write-Host " 3.  INTEL" -ForegroundColor Blue
        Write-Host " 4.  PULAR`n" -ForegroundColor Cyan
        }
        :MainLoop while ($true) {
        Show-Menu
        $choice = Read-Host " "
        ...
        Write-Host "BAIXAR DRIVER DA NVIDIA`n" -ForegroundColor Yellow
...
# download driver
Start-Sleep -Seconds 5
Start-Process "C:\Program Files\BraveSoftware\Brave-Browser\Application\brave.exe" "https://www.nvidia.com/en-us/drivers"
Wait-Process -Name brave

        Write-Host "SELECIONE O DRIVER BAIXADO`n" -ForegroundColor Yellow

# select driver
Start-Sleep -Seconds 5
$InstallFile = Show-ModernFilePicker -Mode File

        Write-Host "DEBLOATANDO DRIVER`n" -ForegroundColor Red
...
Remove-Item "$env:SystemRoot\Temp\NvidiaDriver\NvApp\NvConfigGenerator.dll" -Force -ErrorAction SilentlyContinue | Out-Null
        ...
        Write-Host "INSTALANDO DRIVER`n" -ForegroundColor Green
...
Remove-Item "$env:SystemDrive\NVIDIA" -Recurse -Force -ErrorAction SilentlyContinue | Out-Null
        ...
        Write-Host "IMPORTANDO SETTINGS`n" -ForegroundColor Blue
...
Start-Process -wait "$env:SystemRoot\Temp\Inspector\nvidiaProfileInspector.exe" -ArgumentList "-silentImport -silent $env:SystemRoot\Temp\Inspector.nip"
        ...
        Write-Host "BAIXAR DRIVER DA AMD`n" -ForegroundColor Yellow
...
# wait incase driver timeout or installer bugs
        ...
        80..0 | % { Write-Host "`rIMPORTANDO SETTINGS $_   " -NoNewline; Start-Sleep 1 }; Write-Host "`n" -ForegroundColor Blue
...
cmd /c "reg add `"HKCU\Software\AMD\CN\VirtualSuperResolution`" /v `"AlreadyNotified`" /t REG_DWORD /d `"1`" /f >nul 2>&1"
        ...
        Write-Host "BAIXAR DRIVER DA INTEL`n" -ForegroundColor Yellow
...
cmd /c "reg add `"$regPath`" /v `"Global_LowLatency`" /t REG_DWORD /d `"0`" /f >nul 2>&1"
        ...
        break MainLoop
          ...
          Write-Host "Entrada invalida. Selecione uma opcao valida (1-4).`n" -ForegroundColor Yellow
          Pause
          Show-Menu
          }
          }
        ...
        Clear-Host
        Write-Host "DEFINIR" -ForegroundColor Yellow
        Write-Host "- SOM" -ForegroundColor Yellow
        Write-Host "- RESOLUCAO" -ForegroundColor Yellow
        Write-Host "- HZ MONITOR" -ForegroundColor Yellow
        Write-Host "- MONITOR PRINCIPAL`n" -ForegroundColor Yellow
		## shell:appsFolder\NVIDIACorp.NVIDIAControlPanel_56jybvy8sckqj!NVIDIACorp.NVIDIAControlPanel
    	## ms-settings:display
		## mmsys.cpl
...
Set-ItemProperty -Path "registry::$setreg" -Name 'IsPromoted' -Value 1 -Force
        ...
        Write-Host "PLANO DE ENERGIA`n" -ForegroundColor Green
        ## powercfg.cpl
...
powercfg /setdcvalueindex 99999999-9999-9999-9999-999999999999 de830923-a562-41af-a086-e3a2c6bad2da e69653ca-cf7f-4f05-aa73-cb833fa90ad4 0x00000000 2>$null
        ...
        Write-Host "TIMER RESOLUTION`n" -ForegroundColor Blue
        ## services.msc
...
Get-AppxPackage -allusers *Microsoft.OutlookForWindows* | Remove-AppxPackage -ErrorAction SilentlyContinue
		...
		Write-Host "LIMPEZA DE DISCO`n" -ForegroundColor Yellow
		## cleanmgr.exe
		## %temp%
		## temp
...
Remove-Item "$env:SystemDrive\DumpStack.log" -Force -ErrorAction SilentlyContinue | Out-Null
        ...
        Write-Host "PONTO DE RESTAURACAO`n" -ForegroundColor Cyan
...
cmd /c "reg delete `"HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\SystemRestore`" /v `"SystemRestorePointCreationFrequency`" /f >nul 2>&1"
        ...
        Write-Host "REINICIANDO`n" -ForegroundColor Red

# restart
Start-Sleep -Seconds 5
shutdown -r -t 00
'@
Set-Content -Path "$env:SystemRoot\Temp\madrugaexec2.ps1" -Value $madrugaexec2Ps1 -Force

# edit madrugaexec2.ps1
$Editmadrugaexec2Ps1 = "$env:SystemRoot\Temp\madrugaexec2.ps1"
(Get-Content $Editmadrugaexec2Ps1 -Raw) -replace "``'@","'@" | Set-Content $Editmadrugaexec2Ps1 -NoNewline

# install runonce madrugaexec2 ps1 file to run in normal boot
cmd /c "reg add `"HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\RunOnce`" /v `"madrugaexec2`" /t REG_SZ /d `"powershell.exe -nop -ep bypass -WindowStyle Maximized -f $env:SystemRoot\Temp\madrugaexec2.ps1`" /f >nul 2>&1"
