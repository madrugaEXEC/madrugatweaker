...
# =====================================================================
# FINAL - START MENU LAYOUT SELECTION (ADDED CUSTOM BLOCK)
# =====================================================================
...
while ($true) {
    ...
    if ($choice -match '^[1-2]$') {
        switch ($choice) {
            1 {
                Clear-Host
                # Create reg file for new Start Menu
$NewStartMenu = @"
Windows Registry Editor Version 5.00
...
"AllAppsViewMode"=dword:00000002
"@
                Set-Content -Path "$env:SystemRoot\Temp\NewStartMenu.reg" -Value $NewStartMenu -Force
                Start-Process -Wait "regedit.exe" -ArgumentList "/S `"$env:SystemRoot\Temp\NewStartMenu.reg`"" -WindowStyle Hidden
                Clear-Host
                Exit
            }
            2 {
                Clear-Host
                # Create reg file for old Start Menu
$OldStartMenu = @"
Windows Registry Editor Version 5.00
...
[HKEY_LOCAL_MACHINE\SYSTEM\Control\FeatureManagement\Overrides\14\762256525]
"EnabledState"=-
